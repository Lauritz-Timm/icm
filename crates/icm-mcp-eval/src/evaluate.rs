...
impl Runner {
        ...
        F: FnOnce(&mut McpClient, &FixtureState, &ScenarioSandbox) -> Result<Value>,
    {
        ...
        // Keep process-backed clients for isolation probes (which must prove
        // child env/cwd boundaries), legacy byte-exact wire goldens, and
        // transport/fault-injection lanes. Modern ordinary MCP scenarios
        // drive the integrated production service directly.
        let process_only_transport = sandbox_id.starts_with("legacy.")
            || sandbox_id.contains("invalid-json")
            || sandbox_id.contains("oversized-line");
        let use_in_process =
            !sandbox_id.starts_with("iso.") && !config.fault_database && !process_only_transport;
        let mut client = if use_in_process {
            McpClient::spawn_direct(&sandbox, config.compact)?
        } else {
...
fn assert_proxy_transport_headers(
    ...
    if headers.get("mcp-protocol-version").and_then(Value::as_str) != Some(protocol_version)
        ...
        || (protocol_version != "2026-07-28" && headers.get("mcp-name").is_some())
        || (protocol_version == "2026-07-28"
            && headers.get("mcp-name").and_then(Value::as_str) != Some(""))
